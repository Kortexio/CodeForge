...
export class DiffPreview {
    ...
    static async show(changes: FileChange[]): Promise<{ accepted: string[]; rejected: string[] }> {
        ...
        return new Promise((resolve) => {
            let settled = false;
            const finish = (result: { accepted: string[]; rejected: string[] }) => {
                if (settled) return;
                settled = true;
                resolve(result);
            };
            ...
            panel.webview.onDidReceiveMessage((message) => {
                if (message.type === 'accept') {
                    finish({ accepted: message.files ?? [], rejected: [] });
                    panel.dispose();
                } else if (message.type === 'reject') {
                    finish({ accepted: [], rejected: changes.map(c => c.path) });
                    panel.dispose();
    ...
    private static getHtmlContent(changes: FileChange[]): string {
        const changesHtml = changes.map((change, index) => `
            <div class="file-change" data-index="${index}" data-path="${DiffPreview.escapeAttr(change.path)}">
                <div class="file-header">
                    <input type="checkbox" id="file-${index}" checked data-path="${DiffPreview.escapeAttr(change.path)}" />
                    <label for="file-${index}">
                        <span class="change-type ${change.type}">${change.type}</span>
                        <span class="file-path">${DiffPreview.escapeHtml(change.path)}</span>
                        ${change.oldPath ? `<span class="old-path">(from ${DiffPreview.escapeHtml(change.oldPath)})</span>` : ''}
                    </label>
                </div>
                ${change.newContent ? `
...
<body>
    ...
    private static escapeAttr(text: string): string {
        return DiffPreview.escapeHtml(text).replace(/\n/g, ' ');
    }
}
